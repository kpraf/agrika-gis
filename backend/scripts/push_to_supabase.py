...
# Calamba placeholder (generated by gen_calamba_placeholder.py): observed yield,
# Environment features and predictions for 12 barangays, 2020-2025. Pushed as one
# self-contained step so it doesn't re-touch the real-data cities.
CALAMBA_YIELD_CSV = "db/barangay_yield_city-of-calamba.csv"
CALAMBA_FEATURE = ("db/weather_barangay_calamba_monthly.csv",
                   "db/satellite_barangay_calamba_monthly.csv", "City of Calamba")
CALAMBA_PRED_CSV = "db/barangay_cnn_lstm_predictions_calamba.csv"
CALAMBA_SOURCE = "Calamba CAO (Harvesting Accomplishment Report)"
...
def main():
    ...
    ap.add_argument("--with-calamba", action="store_true",
                    help="Also load the Calamba placeholder: observed yield + Environment + predictions.")
    ...
    if args.with_calamba:
        for rel in [CALAMBA_YIELD_CSV, CALAMBA_PRED_CSV, CALAMBA_FEATURE[0], CALAMBA_FEATURE[1]]:
            if not os.path.exists(os.path.join(BACKEND, rel)):
                sys.exit(f"ERROR: {rel} not found. Run gen_calamba_placeholder.py first. Nothing was done.")
    if not (do_predictions or args.with_features or args.with_barangays
            or args.with_barangay_model or args.with_calamba):
    ...
    if args.with_calamba:
        print("  - UPSERT Calamba placeholder: barangay_yield + Environment + barangay_predictions")
    ...
    if args.with_calamba:
        run("add_barangay_yield_table.py", [], env)
        run("load_barangay_yield.py", ["--csv", CALAMBA_YIELD_CSV, "--source", CALAMBA_SOURCE], env)
        run("add_barangay_feature_tables.py", [], env)
        run("load_barangay_feature_tables.py",
            ["--weather", CALAMBA_FEATURE[0], "--satellite", CALAMBA_FEATURE[1],
             "--municipalities", CALAMBA_FEATURE[2]], env)
        run("load_barangay_predictions.py", ["--csv", CALAMBA_PRED_CSV], env)
    if do_predictions:
